...
class TakeawayList extends StatelessWidget {
  final String city;
  const TakeawayList({Key key, this.city}) : super(key: key);
...
class _ItemsListsState extends State<ItemsLists> {
  ...
  load() async {
    try {
      await api.getTakeawayItemLists(city).then((value) {
        setState(() {
          isLoaded = true;
        });
      });
  ...
  Widget build(BuildContext context) {
    if (isLoaded) {
      return FutureBuilder<List<RestaurantProfileModel>>(
        future: api.getTakeawayItemListsFromSharedPref(),
